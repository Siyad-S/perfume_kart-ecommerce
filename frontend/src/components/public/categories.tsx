        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {categoriesData?.map((category: Category) => (
              <div
                key={category._id}
                className="category-card group relative h-[300px] md:h-[400px] w-full overflow-hidden rounded-2xl cursor-pointer max-md:!opacity-100 max-md:!translate-y-0"
                onClick={(e) => { 
                  e.preventDefault();
                  props?.from === "home"
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 p-6 md:p-8 w-full transform transition-transform duration-500 group-hover:-translate-y-2">
                  ...
                  {category.description && (
                    <p className="text-white/80 text-sm md:text-base line-clamp-2 max-w-[90%] transform translate-y-0 md:translate-y-4 !opacity-100 md:!opacity-0 group-hover:translate-y-0 group-hover:!opacity-100 transition-all duration-500 delay-100">
                      {category.description}
                    </p>
                  )}
                  <div className="mt-4 flex items-center gap-2 text-white/90 text-sm font-medium !opacity-100 md:!opacity-0 group-hover:!opacity-100 transition-opacity duration-500 delay-200">
